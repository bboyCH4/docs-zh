...
对象存储系统提供兼容S3的对象存储接口。它使得ADLS成为一个可以将两种通用类型接口进行融合的存储（POSIX和S3兼容接口）。可以使用户使用原生的Amazon S3 SDK操作ADLS中的文件。
...
ObjectNode是一种无状态设计，具有很高的可扩展性，能够直接操作ADLS集群中存储的所有文件，而无需任何卷装入操作。 *暂不支持纠删码卷*
...
用户
--------------
在使用对象存储功能前，需要先通过资源管理器创建用户。创建用户的同时，会为每个用户生成 *AccessKey* 和 *SecretKey* ，其中 *AccessKey* 是整个ADLS集群中唯一的16个字符的字符串。

ADLS以卷的 **Owner** 字段作为用户ID。创建用户的方式有两种：
...
当用户使用对象存储功能进行某种操作时，ADLS会鉴别该用户是否拥有当前操作的权限。
...
不过ADLS是基于POSIX设计的。根据语义转换规则，对象名'*a/b/c*'中的'*b*'部分转换为文件夹'*a*'下的文件夹'*b*'，对象名'*a/b*'中的'*b*'部分转换为文件夹'*a*'下的文件'*b*'。

类似于上面这样的对象名称在ADLS中是冲突的。
